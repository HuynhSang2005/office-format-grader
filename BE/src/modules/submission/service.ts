import type { ParsedWordData, Paragraph, TextRun } from '../word/types.ts';
import type { ParsedPowerPointFormatData, FormattedSlide, Shape } from '../powerpoint/types.ts';
import { DateTime } from 'luxon';
import { isParagraph, isTable, isTextRun } from '../word/types.ts';
...
export async function analyzeSubmission(submissionFile: File, rubricFile?: File) {
  ...
  try {
    ...
    await fs.writeFile(submissionPath, Buffer.from(await submissionFile.arrayBuffer()));
    const ext = path.extname(submissionFile.name).toLowerCase();
    let submissionRaw;
    let type: 'docx' | 'pptx';
    if (ext === '.docx') { submissionRaw = await parseWordWithFormat(submissionPath); type = 'docx'; }
    else if (ext === '.pptx') { submissionRaw = await parsePowerPointFormat(submissionPath); type = 'pptx'; }
    else { throw new Error('Chỉ hỗ trợ định dạng file .docx và .pptx'); }

    let rubricData: any;
    if (rubricFile) {
      const rubricPath = path.join(tempDir, rubricFile.name);
      await fs.writeFile(rubricPath, Buffer.from(await rubricFile.arrayBuffer()));
      rubricData = { paragraphs: ['Rubric content would be parsed here'] };
    }

    return createSubmissionSummary([
      { filename: submissionFile.name, type, rawData: submissionRaw }
    ], rubricFile ? { filename: rubricFile.name, rawData: rubricData } : undefined);
  } finally {
