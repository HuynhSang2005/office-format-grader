...
import type { ParsedWordData, Paragraph, TextRun, Table, TableCell, ImageRun } from '../../types/word/wordFormat.types';

function parseParagraph(pNode: any, relationships: Map<string, string>): Paragraph {
    ...
    for (const r of runsXml) {
      ...
      const drawing = r['w:drawing']?.[0];
        ...
        if (drawing) {
            ...
            const embedId = drawing['wp:inline']?.[0]?.['a:graphic']?.[0]?.['a:graphicData']?.[0]?.['pic:pic']?.[0]?.['pic:blipFill']?.[0]?.['a:blip']?.[0]?.$?.['r:embed'];
...
export async function parseWordWithFormat(filePath: string): Promise<ParsedWordData> {
    try {
        ...
        // Đọc file document.xml.rels để lấy các relationships
        const relsEntry = zip.getEntry('word/_rels/document.xml.rels');
        if (!relsEntry) throw new Error('File document.xml.rels không tồn tại...');

        const relsContent = relsEntry.getData().toString('utf-8');
        const parsedRels = await parseStringPromise(relsContent);

        // Tạo map để lưu trữ các relationships
        ...
        for (const element of bodyChildren) {
            ...
            if (element['#name'] === 'w:p') {
                extractedContent.push(parseParagraph(element, relationships));
            } else if (element['#name'] === 'w:tbl') {
                extractedContent.push(parseTable(element, relationships));
            }
        }
